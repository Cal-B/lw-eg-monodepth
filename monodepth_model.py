...
class MonodepthModel(object):
    ...
    def post_proc_init(self, reserve_rate=0.05, plus_reserve_rate=0.01):
        ...
        self.l_mask_plus = 1.0 - np.clip(20 * (l - plus_reserve_rate), 0, 1)
    ...
    # Atrous helper
    def AtrousSpatialPyramidPoolingModule(self, inputs, depth=256):
        """
    ...
    def build_vggASPP(self):
        ...
        with tf.variable_scope('encoder'):
            ...
            pool4 = self.maxpool(conv4,                    3) # H/32 
            conv5 = self.AtrousSpatialPyramidPoolingModule(pool4) # H/32
        ...
        with tf.variable_scope('decoder'):
            upconv5 = upconv(conv5, 256, 3, 2) #H/16
    ...
    def build_resASPP(self):
        ...
        with tf.variable_scope('encoder'):
            ...
            pool3 = self.maxpool(conv3,           3) # H/32  -   64D               
            conv4 = self.AtrousSpatialPyramidPoolingModule(pool3) # H/32            
        ...
        with slim.arg_scope([slim.conv2d, slim.conv2d_transpose], activation_fn=tf.nn.elu):
            with tf.variable_scope('model', reuse=self.reuse_variables):
                ...
                if self.params.encoder == 'vgg':
                    ...
                    self.build_resnet50()
                elif self.params.encoder == 'vggASPP':
                    self.build_vggASPP()                    
                elif self.params.encoder == 'resASPP':
                    self.build_resASPP()                     
                else:
                    return None
    ...
    def build_outputs(self):
        ...
        if self.mode == 'test':
            #print('self.disp1[0,:,:,0].shape:', self.disp1[0,:,:,0].get_shape())
            self.disp_est = self.disp1[0,:,:,0]
            self.disp_est_pp = self.post_process(self.disp1)
            self.disp_est_ppp = self.post_process_plus(self.disp1)            
            return
    ...
    def build_losses(self):
        with tf.variable_scope('losses', reuse=self.reuse_variables):
            ...
            # TOTAL LOSS
            self.total_loss = self.params.appearance_loss_weight * self.image_loss + self.params.disp_gradient_loss_weight * self.disp_gradient_loss + self.params.lr_loss_weight * self.lr_loss
